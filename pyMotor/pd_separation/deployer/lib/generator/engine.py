...
from lib.utils import (
    ...
    modify_log_mount, obtain_engine_instance_total, apply_node_selector_override
)
...
def apply_engine_node_selector_overrides(pod_spec, deploy_config, node_type):
    selector_key = {C.NODE_TYPE_P: C.PREFILL_NODE_SELECTOR,
                    C.NODE_TYPE_D: C.DECODE_NODE_SELECTOR}.get(node_type)
    if not selector_key:
        return
